...
page_title: "VMware vSphere: vsphere_supervisor-v2"
...
Some attributes are only available in vSphere 9, consult the documentation if you want to use this with vSphere 8.
Update and Import operations are not supported yet.
...
* `namespace` - (Optional) List of namespaces in the Supervisor cluster. See [namespace](#nestedblock--namespace).
...
* `count` - (Optional) The number of control plane VMs to deploy.
...
* `certificate_chain` - (Required) The certificate authority chain of the image registry.
