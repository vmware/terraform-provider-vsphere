...
func TestAccResourceVSphereConfigProfile_debug(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			RunSweepers()
			testAccPreCheck(t)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceVSphereConfigProfileConfigDebug(),
			},
			{
				Config: testAccResourceVSphereConfigProfileConfigDebug2(),
			},
			{
				Config: testAccResourceVSphereConfigProfileConfigDebug(),
			},
...
func testAccResourceVSphereConfigProfileConfigDebug() string {
	return fmt.Sprintf(`
%s
...
resource "vsphere_configuration_profile" "profile1" {
  configuration     = file("~/Downloads/cluster1.json")
  cluster_id        = vsphere_compute_cluster.cluster1.id
}
`,
		testhelper.ConfigDataRootDC1(),
...
func testAccResourceVSphereConfigProfileConfigDebug2() string {
	return fmt.Sprintf(`
%s
