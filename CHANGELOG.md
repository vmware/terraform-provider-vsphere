...
- `r/datacenter_network_protocol_profile`: New resource to manage vSphere network protocol profiles (IP pools). Validates that configured `network_ids` are not already assigned to another network protocol profile, since vCenter Server would otherwise silently move them.
