...
func TestAccDataSourceVSphereTag_byID(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			RunSweepers()
			testAccPreCheck(t)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceVSphereTagByIDConfig(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"data.vsphere_tag.testacc-tag-data-by-id",
						"name",
						testAccDataSourceVSphereTagConfigName,
					),
					resource.TestCheckResourceAttr(
					...
					resource.TestCheckResourceAttrPair(
						"data.vsphere_tag.testacc-tag-data-by-id", "id",
						"vsphere_tag.testacc-tag", "id",
					),
					resource.TestCheckResourceAttrPair(
						"data.vsphere_tag.testacc-tag-data-by-id", "category_id",
						"vsphere_tag_category.testacc-category", "id",
					),
				),
			},
...
func testAccDataSourceVSphereTagByIDConfig() string {
	return fmt.Sprintf(`
variable "tag_category_name" {
  default = "%s"
}
